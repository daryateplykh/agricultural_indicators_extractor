...
class ScannedExtractorMistral:
    ...
    def extract_text_mistral(self, image: PILImage.Image, year: str, page_num: int = 0) -> str:
        ...
        ocr_response_left = self.mistral_client.ocr.process(
            ...
            document={"type": "image_url", "image_url": f"data:image/jpeg;base64,{base64image_left}"}
        )
        mistral_text_left = ocr_response_left.pages[0].markdown
        

        ocr_response_right = self.mistral_client.ocr.process(
            model="mistral-ocr-latest",
            document={"type": "image_url", "image_url": f"data:image/jpeg;base64,{base64image_right}"}
        )
        mistral_text_right = ocr_response_right.pages[0].markdown
